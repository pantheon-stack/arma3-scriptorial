/*
	...
	Description:
	Export mission to SQF script. When called, the script will dynamically recreate the mission.
			...
			if (_property != "") then {
				...
				if (_expression != "") then {
					...
					if !(isnil "_value") then {
						...
						//--- Format attribute value (don't use 'format' comamnd, it has a character limit)
						...
						//};
						//format [_expression + ";",configname _x,str _value] call _fnc_addLine;
					};
				};
			};
...
//--- Header
format ["// Export of '%1.%2' by %3 on v%4",_missionname,worldname,profilename,VERSION] call _fnc_addLine;
if (_isRelative) then {
...
"private _objectIDs = [];" call _fnc_addLine;
{
	...
	if (_simulation != "uavpilot") then { //--- Ignore virtual UAV crews, they cannot be created by createUnit anyway. createVehicleCrew is used instead on UAVs.
